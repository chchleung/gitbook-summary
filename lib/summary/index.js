...
function init(options) {
    ...
    wiki_ignore = options.wiki_ignore || bookConfig.wiki_ignore;   // add by lzc
}
        ...
        _.forEach(filesObj, function(n, key) {
            if (!_.includes(ignores, key)) {
                if (_.isObject(n)) {
                    ...
                    // 作为下面修改title的辅助检测by lzc --- start ---
                    var obkey = Object.keys(n);
                    var theR = null;
                    obkey.forEach((item)=>{
                    ...
                    // add by lzc 主要是为了如果将Readme的title更改，在目录层级也跟着改 --- start----
                    else if (theR) {
                        var readmeDir = n[theR];
                        desc += _.repeat(' ', step) + formatCatalog(theR.slice(3), '-') + readmeDir;
                        skip = theR;
                    }
                    // add by lzc 主要是为了如果将Readme的title更改，在目录层级也跟着改 --- end ----
...
function getFiles(root) {
    ...
    readFile(root, filesJson, sortedBy, wiki_ignore);  // modify by lzc
...
function isSkiped(key, skip) {
    var result = !_.isEmpty(skip) && _.isEqual(key.toLowerCase(), skip.toLowerCase()) || _.isEqual(key.toLowerCase(), 'readme') || key.startsWith('_R_');  // modify by lzc
    return result;
}
